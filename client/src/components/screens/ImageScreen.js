...
const ImageScreen = () => {
    ...
    const imageHandler = async (e) => {
        ...
        try {
          const { data } = await axios.post("/imageGen", {text});
          setImage(data);
        } catch (err) {
    ...
    return (
        ...
        <form onSubmit={imageHandler}>
            <Typography variant="h3" mb={2}>Image Generator</Typography>
            <Stack direction="row" spacing={1}>
                <Box width="87%">
                    <TextField multiline ="true" placeholder ="Enter image concept here:" margin="normal" required fullWidth value ={text} onChange={(e) => setText(e.target.value)}/>
                </Box>  
        ...
        <Box sx={{ display: 'flex', justifyContent: "center", my:5 }}>
            <img src={image} alt="A.I. Generated"></img>
        </Box>

...
const JavaScriptScreen = () => {
    ...
    const codeHandler = async (e) => {
        ...
        try {
          const { data } = await axios.post("/jsConvert", {text});
          setCode(data);
        } catch (err) {
            console.error("Error during code request:", err);
    ...
    return (
        ...
        <form onSubmit={codeHandler}>
            ...
            <Stack direction="row" spacing={1}>
                <Box width="87%">
                    <TextField multiline ="true" placeholder ="Enter instructions:" margin="normal" required fullWidth value ={text} onChange={(e) => setText(e.target.value)}/>
                </Box>  
        ...
        : <Card sx={{mt:4, p:2, border: 1, boxShadow:0, borderColor:"neutral.medium", borderRadius: 2, height: "500px", bgcolor: "background.default"}}>
                <Typography variant="h5" color="neutral.main" sx={{textAlign: "center", verticalAlign:"middle", lineHeight:"450px"}}>Code will appear here:</Typography>
            </Card>}
